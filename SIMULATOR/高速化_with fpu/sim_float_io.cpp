...
string s = "/Users/maimai/my-3A/cpu-simu/simulator/高速化ver/sim_result.txt";
ofstream ofs(s);
...
//     map<float, float> mf; 
// }
vector<vector<int64_t> > L1(SIZE); //SIZE/10
...
int main(int argc, char *argv[]){
    ...
    while(pc < pc_size){
        ...
        if(con_label.count(pc)){
            if(debug) cout << '[' << con_label[pc] << "] (" << pc << ')' <<endl;
            // if((con_label[pc]).find(break_label) != std::string::npos){
            //     // cout << "yes" << endl;
            //     break;
            // }
        ...
        switch(op.conv_type(op_num)){
            ...
            case B:
                ...
                // cout << "pc" << pc << endl;
                // jumpの処理？
                if(debug) cout << "  #B " << rd << " "<< rs1 <<" " << rs2 << endl;
                instr_count++; //?
                continue;
            case J:
                imm = opline.imm.i;
                ...
                reg_val.at(opline.rd) = rd; 
                // if(break_symbol == true) 
                if(debug) cout << "  #J " << rd << " "<< rs1 <<" " << rs2 << endl;
