...
Bit32 fsub(Bit32 x1, Bit32 x2){ //float x_1, float x_2){
    ...
    // 7, 8
    Bit32 de, sel;
    de  = (sub_uint(tde,7,5) > 0) ? 31 : sub_uint(tde,4,0) ; //tde  > 31
    // m1aの方が大きければsel==0
    ...
    // 19, 20
    Bit32 myr, eyri;
    // cout << myf << " " << stck << " " << (s1==s2) << " "<< (sub_uint(myf,26,2) + (1<<25)-1) << " "<< sub_uint(myf,26,2);
    myr = ((at_uint(myf,1) == 1 && at_uint(myf,0)== 0 && stck == 0 && at_uint(myf,2) == 1 ) || ( at_uint(myf,1) == 1 && at_uint(myf,0) == 0 && (s1 == s2) && stck == 1 ) || ( at_uint(myf,1) == 1 && at_uint(myf,0) == 1 ) ) ? (sub_uint(myf,26,2) + 1) : sub_uint(myf,26,2); //(1<<25)-1
    // ここまでok
    eyri = eyr + (1 << 8) - 1;//>> 8;
    // 21
    ...
    // cout << e1 << " " << e2 << " " << nzm1 << " " << nzm2 << " " << (s1==s2) << endl;
...
//     Bit32 y = fsub(f_to_bit(x_1), f_to_bit(x_2));
//     // Bit32 x1,x2,y;
//     // x1 = 0x43fa3146;
//     // x2 = 0x45fa4345;
//     // y = fsub(x1,x2);
