...
struct Op {
    ...
    int conv_type(int op){
        ...
        else if (op == 10 || op == 11)  {cout << "type O" << endl; return -1;}
        else _type = F;
...
int main(int argc, char *argv[]){
    ...
    vector<int64_t> fop_load(pc_size+4, -1);//
    // vector<int64_t> op_haz(pc_size+4);//
        ...
        switch(op.conv_type(op_num)){
            case R: //33-46
                ...
                if(!fast_mode){
                    ...
                    // if(op_haz.at(pre_pc/4) == opline.rs1 || op_haz.at(pre_pc/4) == opline.rs2){
                    //     clock_count += 3;
                    // }
                    // op_haz.at(pre_pc) = opline.rd;
                } 
                if(op_num == 33){ //add
                    rd=rs1+rs2;
                    // pc += 4; //最後に
            ...
            case I_1:
                ...
                if(!fast_mode){ //op_load.at(pre_pc/4)){
                    ...
                    // if(line.find(pre_rd) < 100 && pre_rd != a0){
                        // cout << "hazard" << op_load[pre_pc] << endl;
                        // ofs2 << "load_hazard " << opline.rs1 << endl;
                        clock_count++;
                    }
                }
                pre_pc = pc;
            ...
            case F:
                ...
                if(op_num == 81){ //"flw"){
                    ...
                    if(!fast_mode){
                        if(op_load.at(pre_pc/4) == opline.rs1 || fop_load.at(pre_pc/4) == opline.rs2){
                            clock_count++;
                        }
                    }
                    rs1 = reg_val.at(opline.rs1);//reg_index(a2);
                ...
                if(!fast_mode){
                    if(fop_load.at(pre_pc/4) ==  opline.rs1|| fop_load.at(pre_pc/4) == opline.rs2 || fop_load.at(pre_pc/4) == opline.rs3){
                        clock_count++;
                    }
                }
                // }
    ...
    if(!fast_mode) cout << fixed << setprecision(10) << hit_count/(hit_count+miss_count) << endl;
    cout << "clean dirty " << clean_miss << " " << dirty_miss << endl;
    cout << "実行命令数 " << instr_count << endl;
    clock_count = clock_count = instr_count + clock_count + hit_count*3 + clean_miss*3 +dirty_miss*8;//instr_count + clock_count;
