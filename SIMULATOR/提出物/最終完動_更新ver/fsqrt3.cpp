...
Bit32 fsqrt(Bit32 x){
    ...
    // in_data(filename);
    Bit64 data_from_coe = array1[addra]; //arrayから撮ってきたもの？ 
    ...
    Bit64 delta_y = delta_y_h + delta_y_l; //((Bit64)gradient * delta_x)  % (1ll << 49);
...
// int main(){
//     // string filename = "fsqrt_data.coe"; //inv
...
//     float x_1 = 0;
    ...
    //     Bit32 y = fsqrt(f_to_bit(x_1));
    //     float f_y = bit_to_float(y);
    //     float ans = pow(x_1, 0.5);
    //     float diff = abs(f_y - ans);
    //     // if(diff > pow(2, -126)){
    //     //     int max = max(abs(x_1)*pow(2,-23), abs(x_2)*pow(2,-23))
    //     // }
    //     float max = f_max(abs(pow(x_1, 0.5))*pow(2,-20),  pow(2,-126));
    //     // cout << diff << " " << max << endl;
    //     if(diff > max){
