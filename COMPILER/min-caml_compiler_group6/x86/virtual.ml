...
let rec g env = function (* 式の仮想マシンコード生成 (caml2html: virtual_g) *)
  ...
  | Closure.Var(x, n) ->
      ...
      | _ -> Ans(Mov(x), n))
  | Closure.MakeCls((x, t), { Closure.entry = l; Closure.actual_fv = ys }, e2, n) -> (* クロージャの生成 (caml2html: virtual_makecls) *)
  ...
  | Closure.Tuple(xs, n) -> (* 組の生成 (caml2html: virtual_tuple) *)
      let y = Id.genid "t" in
      let (offset, store) =
        expand
  ...
  | Closure.LetTuple(xts, y, e2, n) ->
      ...
      load
  | Closure.Get(x, y, n) -> (* 配列の読み出し (caml2html: virtual_get) *)
      (match M.find x env with
      | Type.Array(Type.Unit) -> Ans(Nop, n)
      | Type.Array(Type.Float) -> Ans(LdDF(x, V(y), n), n)
      | Type.Array(_) -> Ans(Ld(x, V(y), n), n)
      | _ -> assert false)
  | Closure.Put(x, y, z, n) ->
      (match M.find x env with
      | Type.Array(Type.Unit) -> Ans(Nop, n)
      | Type.Array(Type.Float) -> Ans(StDF(z, x, V(y), n), n)
      | Type.Array(_) -> Ans(St(z, x, V(y), n), n)
      | _ -> assert false)
