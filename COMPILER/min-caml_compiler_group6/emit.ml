...
and g' oc = function (* ��̿��Υ�����֥����� (caml2html: emit_gprime) *)
  ...
  | NonTail(_), Nop, _ -> ()
  | NonTail(x), Set(i), n -> Printf.fprintf oc "\taddi\t%s, %s, %d  #%d\n" x reg_zero i n
  | NonTail(x), SetL(Id.L(y)), n -> Printf.fprintf oc "\tmv\t%s, %s  #%d\n" x y n
  | NonTail(x), Mov(y), _ when x = y -> ()
  | NonTail(x), Mov(y), n -> Printf.fprintf oc "\tmv\t%s, %s  #%d\n" x y n
  | NonTail(x), Neg(y), n -> Printf.fprintf oc "\tsub\t%s, %s, %s  #%d\n" x reg_zero y n
  | NonTail(x), Add(y, V z), n -> Printf.fprintf oc "\tadd\t%s, %s, %s  #%d\n" x y z n
  | NonTail(x), Add(y, C i), n -> Printf.fprintf oc "\taddi\t%s, %s, %d  #%d\n" x y i n
  | NonTail(x), Sub(y, V z), n -> Printf.fprintf oc "\tsub\t%s, %s, %s  #%d\n" x y z n
  | NonTail(x), Sub(y, C i), n -> Printf.fprintf oc "\taddi\t%s, %s, %d  #%d\n" x y (-i) n
  | NonTail(x), Xor(y, V z), n -> Printf.fprintf oc "\txor\t%s, %s, %s  #%d\n" x y z n
  | NonTail(x), Xor(y, C i), n -> Printf.fprintf oc "\txori\t%s, %s, %d  #%d\n" x y i n
  | NonTail(x), Or(y, V z), n -> Printf.fprintf oc "\tor\t%s, %s, %s  #%d\n" x y z n
  | NonTail(x), Or(y, C i), n -> Printf.fprintf oc "\tori\t%s, %s, %d  #%d\n" x y i n
  | NonTail(x), And(y, V z), n -> Printf.fprintf oc "\tand\t%s, %s, %s  #%d\n" x y z n
  | NonTail(x), And(y, C i), n -> Printf.fprintf oc "\tandi\t%s, %s, %d  #%d\n" x y i n
  | NonTail(x), SLL(y, V z), n -> Printf.fprintf oc "\tsll\t%s, %s, %s  #%d\n" x y z n
  | NonTail(x), SLL(y, C i), n -> Printf.fprintf oc "\tslli\t%s, %s, %d  #%d\n" x y i n
  | NonTail(x), SRL(y, V z), n -> Printf.fprintf oc "\tsrl\t%s, %s, %s  #%d\n" x y z n
  | NonTail(x), SRL(y, C i), n -> Printf.fprintf oc "\tsrli\t%s, %s, %d  #%d\n" x y i n
  | NonTail(x), Ld(y, z', _), n -> Printf.fprintf oc "\tlw\t%s, %s(%s)  #%d\n" x (pp_id_or_imm z') y n
  | NonTail(_), St(x, y, z', _), n -> Printf.fprintf oc "\tsw\t%s, %s(%s)  #%d\n" x (pp_id_or_imm z') y n
  | NonTail(x), FAbs(y), n ->
  ...
  | NonTail(x), FNegD(y), n ->
      ...
      if x <> y then Printf.fprintf oc "\tmv\t%s, %s  #%d\n" (co_freg x) (co_freg y) n
  | NonTail(x), FAddD(y, z), n -> Printf.fprintf oc "\tfadd\t%s, %s, %s  #%d\n" x y z n
  | NonTail(x), FSubD(y, z), n -> Printf.fprintf oc "\tfsub\t%s, %s, %s  #%d\n" x y z n
  | NonTail(x), FMulD(y, z), n -> Printf.fprintf oc "\tfmul\t%s, %s, %s  #%d\n" x y z n
  | NonTail(x), FDivD(y, z), n -> Printf.fprintf oc "\tfdiv\t%s, %s, %s  #%d\n" x y z n
  | NonTail(x), FtoI(y), n -> Printf.fprintf oc "\tftoi\t%s, %s  #%d\n" x y n
  | NonTail(x), ItoF(y), n -> Printf.fprintf oc "\titof\t%s, %s  #%d\n" x y n
  | NonTail(_), Out(x), n -> Printf.fprintf oc "\tout\t%s  #%d\n" x n
  | NonTail(x), In, n -> Printf.fprintf oc "\tin\t%s  #%d\n" x n
  | NonTail(_), Sethp(x), n -> Printf.fprintf oc "\tmv\t%s, %s  #%d\n" reg_hp x n 
  | NonTail(x), Gethp, n -> Printf.fprintf oc "\tmv\t%s, %s  #%d\n" x reg_hp n
  | NonTail(x), LdDF(y, z', _), n -> Printf.fprintf oc "\tflw\t%s, %s(%s)  #%d\n" x (pp_id_or_imm z') y n
  | NonTail(_), StDF(x, y, z', _), n -> Printf.fprintf oc "\tfsw\t%s, %s(%s) #%d\n" x (pp_id_or_imm z') y n
  ...
  | Tail, IfEq(x, y', e1, e2), n ->
    let y = pp_id_or_imm' oc y' in
      Printf.fprintf oc "\tbeq\t%s, %s, 12  #%d\n" x y n;
      g'_tail_if oc e1 e2 "be"
  | Tail, IfLE(x, y', e1, e2), n ->
    let y = pp_id_or_imm' oc y' in
      Printf.fprintf oc "\tbge\t%s, %s, 12  #%d\n" y x n;
