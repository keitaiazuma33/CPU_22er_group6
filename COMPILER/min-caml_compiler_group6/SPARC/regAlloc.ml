...
let rec alloc dest cont regenv x t =
  ...
  assert (not (M.mem x regenv)); (* すでに割り当て済みでないか確認 *)
  let all =
    match t with
    | Type.Unit -> ["%g0"] (* dummy *)
    | Type.Float -> allfregs
...
let rec g dest cont regenv = function (* 命令列のレジスタ割り当て (caml2html: regalloc_g) *)
  ...
  | Let((x, t) as xt, exp, e, n) ->
      assert (not (M.mem x regenv)); (* α変換により正常ならassertはされないはず *)
      ...
      | Alloc(r) -> (* 無事割り当てられるので普通に割り当てる *)
          let (e2', regenv2) = g dest cont (add x r regenv1) e in
          (concat e1' (r, t) e2', regenv2))
and g'_and_restore dest cont regenv exp n = (* 使用される変数をスタックからレジスタへRestore (caml2html: regalloc_unspill) *)
  try g' dest cont regenv n exp
  with NoReg(x, t) ->
    ((* Format.eprintf "restoring %s@." x; *)
     g dest cont regenv (Let((x, t), Restore(x), Ans(exp, n), n)))
and g' dest cont regenv n = function (* 各命令のレジスタ割り当て (caml2html: regalloc_gprime) *)
  | Nop | Set _ | SetL _ | Comment _ | Restore _ as exp -> (Ans(exp, n), regenv)
...
and g'_if dest cont regenv exp constr e1 e2 n = (* ifのレジスタ割り当て (caml2html: regalloc_if) *)
  ...
  (List.fold_left
     (fun e x ->
       ...
       seq(Save(M.find x regenv, x), e, n)) (* そうでない変数は分岐直前にセーブ *)
     (Ans(constr e1' e2', n))
     (fv cont),
   regenv')
and g'_call dest cont regenv exp constr ys zs n = (* 関数呼び出しのレジスタ割り当て (caml2html: regalloc_call) *)
  (List.fold_left
     (fun e x ->
