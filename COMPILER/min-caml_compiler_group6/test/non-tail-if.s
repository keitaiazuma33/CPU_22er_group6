...
min_caml_start:
	...
	addi	%x4, %x0, 8192
	addi	%x5, %x0, 65536
	fmv	%f0, l.359  #0 pc 688
	sw	%x1, 0(%x2)  #207 pc 692
	addi	%x2, %x2, 4  #207 pc 696
	jal	%x1, min_caml_truncate  #207 pc 700
	addi	%x2, %x2, -4  #207 pc 704
	lw	%x1, 0(%x2) #207 pc 708
	fmv	%f0, l.361  #0 pc 712
	sw	%x6, 0(%x2)  #208 pc 716
	sw	%x1, 4(%x2)  #208 pc 720
	addi	%x2, %x2, 8  #208 pc 724
	jal	%x1, min_caml_truncate  #208 pc 728
	addi	%x2, %x2, -8  #208 pc 732
	lw	%x1, 4(%x2) #208 pc 736
	fmv	%f0, l.363  #0 pc 740
	sw	%x6, 4(%x2)  #209 pc 744
	sw	%x1, 8(%x2)  #209 pc 748
	addi	%x2, %x2, 12  #209 pc 752
	jal	%x1, min_caml_truncate  #209 pc 756
	addi	%x2, %x2, -12  #209 pc 760
	lw	%x1, 8(%x2) #209 pc 764
	bge	%x6, %x0, 12  #211 pc 768
	j	bge_else.393 #pc 772
	nop #pc 776
	lw	%x7, 0(%x2)  #211 pc 780
	j	bge_cont.394 #pc 784
	nop #pc 788
bge_else.393: #pc 792
	lw	%x7, 4(%x2)  #211 pc 792
bge_cont.394: #pc 796
	lw	%x8, 0(%x2)  #212 pc 796
	bge	%x0, %x8, 12  #212 pc 800
	j	ble_else.395 #pc 804
	nop #pc 808
	lw	%x9, 4(%x2)  #212 pc 812
	j	ble_cont.396 #pc 816
	nop #pc 820
ble_else.395: #pc 824
	addi	%x9, %x6, 0  #212 pc 824
ble_cont.396: #pc 828
	add	%x7, %x7, %x9  #211 pc 828
	lw	%x9, 4(%x2)  #213 pc 832
	bge	%x9, %x0, 12  #213 pc 836
	j	bge_else.397 #pc 840
	nop #pc 844
	j	bge_cont.398 #pc 848
	nop #pc 852
bge_else.397: #pc 856
	addi	%x6, %x8, 0  #213 pc 856
bge_cont.398: #pc 860
	add	%x6, %x7, %x6  #211 pc 860
	sw	%x1, 8(%x2)  #210 pc 864
	addi	%x2, %x2, 12  #210 pc 868
	jal	%x1, print_int.180  #210 pc 872
	addi	%x2, %x2, -12  #210 pc 876
	lw	%x1, 8(%x2) #210 pc 880
