...
min_caml_start:
	...
	addi	%x4, %x0, 8192
	addi	%x5, %x0, 65536
	addi	%x6, %x0, 1  #0 pc 264
	addi	%x7, %x0, 2  #0 pc 268
	addi	%x8, %x0, 3  #0 pc 272
	addi	%x9, %x0, 4  #0 pc 276
	addi	%x10, %x0, 5  #0 pc 280
	addi	%x11, %x0, 6  #0 pc 284
	addi	%x12, %x0, 7  #0 pc 288
	addi	%x13, %x0, 8  #0 pc 292
	addi	%x14, %x0, 9  #0 pc 296
	addi	%x15, %x0, 10  #0 pc 300
	addi	%x16, %x3, 0  #214 pc 304
	addi	%x3, %x3, 40  #214 pc 308
	sw	%x15, 36(%x16)  #214 pc 312
	sw	%x14, 32(%x16)  #214 pc 316
	sw	%x13, 28(%x16)  #214 pc 320
	sw	%x12, 24(%x16)  #214 pc 324
	sw	%x11, 20(%x16)  #214 pc 328
	sw	%x10, 16(%x16)  #214 pc 332
	sw	%x9, 12(%x16)  #214 pc 336
	sw	%x8, 8(%x16)  #214 pc 340
	sw	%x7, 4(%x16)  #214 pc 344
	sw	%x6, 0(%x16)  #214 pc 348
	addi	%x6, %x16, 0  #214 pc 352
	sw	%x1, 0(%x2)  #214 pc 356
	addi	%x2, %x2, 4  #214 pc 360
	jal	%x1, h.197  #214 pc 364
	addi	%x2, %x2, -4  #214 pc 368
	lw	%x1, 0(%x2) #214 pc 372
	sw	%x1, 0(%x2)  #214 pc 376
	addi	%x2, %x2, 4  #214 pc 380
	jal	%x1, print_int.177  #214 pc 384
	addi	%x2, %x2, -4  #214 pc 388
	lw	%x1, 0(%x2) #214 pc 392
	sw	%x1, 0(%x2)  #215 pc 396
	addi	%x2, %x2, 4  #215 pc 400
	jal	%x1, min_caml_print_newline  #215 pc 404
	addi	%x2, %x2, -4  #215 pc 408
	lw	%x1, 0(%x2) #215 pc 412
