.section	".rodata"
.align	8
.section	".text"
	nop
	j min_caml_start
mul_abs.175:  #pc 8
	addi	%x31, %x0, -1  #pc 8
	beq	%x8, %x31, 12  #57 pc 12
	j	be_else.428 #pc 16
	nop #pc 20
	addi	%x6, %x9, 0  #58 pc 24
	ret #pc 28
	nop #pc 32
be_else.428: #pc 36
	addi	%x10, %x0, 1  #0 pc 36
	sll	%x10, %x10, %x8  #60 pc 40
	and	%x10, %x7, %x10  #60 pc 44
	beq	%x10, %x0, 12  #60 pc 48
	j	be_else.429 #pc 52
	nop #pc 56
	addi	%x8, %x8, -1  #61 pc 60
	j	mul_abs.175  #61 pc 64
	nop #pc 68
be_else.429: #pc 72
	addi	%x10, %x8, -1  #63 pc 72
	sll	%x8, %x6, %x8  #63 pc 76
	add	%x9, %x9, %x8  #63 pc 80
	addi	%x8, %x10, 0  #0 pc 84
	j	mul_abs.175  #63 pc 88
	nop #pc 92
mul.180:  #pc 96
	bge	%x6, %x0, 12  #67 pc 96
	j	bge_else.430 #pc 100
	nop #pc 104
	addi	%x8, %x6, 0  #67 pc 108
	j	bge_cont.431 #pc 112
	nop #pc 116
bge_else.430: #pc 120
	sub	%x8, %x0, %x6  #67 pc 120
bge_cont.431: #pc 124
	bge	%x7, %x0, 12  #68 pc 124
	j	bge_else.432 #pc 128
	nop #pc 132
	addi	%x9, %x7, 0  #68 pc 136
	j	bge_cont.433 #pc 140
	nop #pc 144
bge_else.432: #pc 148
	sub	%x9, %x0, %x7  #68 pc 148
bge_cont.433: #pc 152
	addi	%x10, %x0, 30  #0 pc 152
	addi	%x11, %x0, 0  #0 pc 156
	sw	%x7, 0(%x2)  #69 pc 160
	sw	%x6, 4(%x2)  #69 pc 164
	addi	%x7, %x9, 0  #0 pc 168
	addi	%x6, %x8, 0  #0 pc 172
	addi	%x9, %x11, 0  #0 pc 176
	addi	%x8, %x10, 0  #0 pc 180
	sw	%x1, 8(%x2)  #69 pc 184
	addi	%x2, %x2, 12  #69 pc 188
	jal	%x1, mul_abs.175  #69 pc 192
	addi	%x2, %x2, -12  #69 pc 196
	lw	%x1, 8(%x2) #69 pc 200
	lw	%x7, 4(%x2)  #70 pc 204
	srli	%x7, %x7, 31  #70 pc 208
	lw	%x8, 0(%x2)  #70 pc 212
	srli	%x8, %x8, 31  #70 pc 216
	beq	%x7, %x8, 12  #70 pc 220
	j	be_else.434 #pc 224
	nop #pc 228
	ret #pc 232
	nop #pc 236
be_else.434: #pc 240
	sub	%x6, %x0, %x6  #73 pc 240
	ret #pc 244
	nop #pc 248
div_abs.183:  #pc 252
	addi	%x31, %x0, -1  #pc 252
	beq	%x8, %x31, 12  #77 pc 256
	j	be_else.435 #pc 260
	nop #pc 264
	addi	%x6, %x9, 0  #78 pc 268
	ret #pc 272
	nop #pc 276
be_else.435: #pc 280
	srl	%x10, %x6, %x8  #80 pc 280
	bge	%x10, %x7, 12  #80 pc 284
	j	ble_else.436 #pc 288
	nop #pc 292
	sll	%x10, %x7, %x8  #81 pc 296
	sub	%x6, %x6, %x10  #81 pc 300
	addi	%x10, %x8, -1  #81 pc 304
	addi	%x11, %x0, 1  #0 pc 308
	sll	%x8, %x11, %x8  #81 pc 312
	add	%x9, %x9, %x8  #81 pc 316
	addi	%x8, %x10, 0  #0 pc 320
	j	div_abs.183  #81 pc 324
	nop #pc 328
ble_else.436: #pc 332
	addi	%x8, %x8, -1  #83 pc 332
	j	div_abs.183  #83 pc 336
	nop #pc 340
div.188:  #pc 344
	bge	%x6, %x0, 12  #87 pc 344
	j	bge_else.437 #pc 348
	nop #pc 352
	addi	%x8, %x6, 0  #87 pc 356
	j	bge_cont.438 #pc 360
	nop #pc 364
bge_else.437: #pc 368
	sub	%x8, %x0, %x6  #87 pc 368
bge_cont.438: #pc 372
	bge	%x7, %x0, 12  #88 pc 372
	j	bge_else.439 #pc 376
	nop #pc 380
	addi	%x9, %x7, 0  #88 pc 384
	j	bge_cont.440 #pc 388
	nop #pc 392
bge_else.439: #pc 396
	sub	%x9, %x0, %x7  #88 pc 396
bge_cont.440: #pc 400
	addi	%x10, %x0, 30  #0 pc 400
	addi	%x11, %x0, 0  #0 pc 404
	sw	%x7, 0(%x2)  #89 pc 408
	sw	%x6, 4(%x2)  #89 pc 412
	addi	%x7, %x9, 0  #0 pc 416
	addi	%x6, %x8, 0  #0 pc 420
	addi	%x9, %x11, 0  #0 pc 424
	addi	%x8, %x10, 0  #0 pc 428
	sw	%x1, 8(%x2)  #89 pc 432
	addi	%x2, %x2, 12  #89 pc 436
	jal	%x1, div_abs.183  #89 pc 440
	addi	%x2, %x2, -12  #89 pc 444
	lw	%x1, 8(%x2) #89 pc 448
	lw	%x7, 4(%x2)  #90 pc 452
	srli	%x7, %x7, 31  #90 pc 456
	lw	%x8, 0(%x2)  #90 pc 460
	srli	%x8, %x8, 31  #90 pc 464
	beq	%x7, %x8, 12  #90 pc 468
	j	be_else.441 #pc 472
	nop #pc 476
	ret #pc 480
	nop #pc 484
be_else.441: #pc 488
	sub	%x6, %x0, %x6  #93 pc 488
	ret #pc 492
	nop #pc 496
print_int.193:  #pc 500
	bge	%x6, %x0, 12  #100 pc 500
	j	bge_else.442 #pc 504
	nop #pc 508
	addi	%x31, %x0, 10  #pc 512
	bge	%x6, %x31, 12  #104 pc 516
	j	bge_else.443 #pc 520
	nop #pc 524
	addi	%x7, %x0, 10  #0 pc 528
	sw	%x6, 0(%x2)  #107 pc 532
	sw	%x1, 4(%x2)  #107 pc 536
	addi	%x2, %x2, 8  #107 pc 540
	jal	%x1, div.188  #107 pc 544
	addi	%x2, %x2, -8  #107 pc 548
	lw	%x1, 4(%x2) #107 pc 552
	sw	%x6, 4(%x2)  #108 pc 556
	sw	%x1, 8(%x2)  #108 pc 560
	addi	%x2, %x2, 12  #108 pc 564
	jal	%x1, print_int.193  #108 pc 568
	addi	%x2, %x2, -12  #108 pc 572
	lw	%x1, 8(%x2) #108 pc 576
	addi	%x7, %x0, 10  #0 pc 580
	lw	%x6, 4(%x2)  #109 pc 584
	sw	%x1, 8(%x2)  #109 pc 588
	addi	%x2, %x2, 12  #109 pc 592
	jal	%x1, mul.180  #109 pc 596
	addi	%x2, %x2, -12  #109 pc 600
	lw	%x1, 8(%x2) #109 pc 604
	lw	%x7, 0(%x2)  #109 pc 608
	sub	%x6, %x7, %x6  #109 pc 612
	addi	%x6, %x6, 48  #109 pc 616
	sw	%x6, 0(%x5)  #109 pc 620
	addi	%x5, %x5, 4  #109 pc 624
	ret #pc 628
	nop #pc 632
bge_else.443: #pc 636
	addi	%x6, %x6, 48  #105 pc 636
	sw	%x6, 0(%x5)  #105 pc 640
	addi	%x5, %x5, 4  #105 pc 644
	ret #pc 648
	nop #pc 652
bge_else.442: #pc 656
	addi	%x7, %x0, 45  #0 pc 656
	sw	%x7, 0(%x5)  #101 pc 660
	addi	%x5, %x5, 4  #101 pc 664
	sub	%x6, %x0, %x6  #102 pc 668
	j	print_int.193  #102 pc 672
	nop #pc 676
g.239:  #pc 680
	lw	%x7, 40(%x29)  #0 pc 680
	lw	%x8, 36(%x29)  #0 pc 684
	lw	%x9, 32(%x29)  #0 pc 688
	lw	%x10, 28(%x29)  #0 pc 692
	lw	%x11, 24(%x29)  #0 pc 696
	lw	%x12, 20(%x29)  #0 pc 700
	lw	%x13, 16(%x29)  #0 pc 704
	lw	%x14, 12(%x29)  #0 pc 708
	lw	%x15, 8(%x29)  #0 pc 712
	lw	%x16, 4(%x29)  #0 pc 716
	add	%x14, %x16, %x14  #211 pc 720
	add	%x13, %x14, %x13  #211 pc 724
	add	%x12, %x13, %x12  #211 pc 728
	add	%x11, %x12, %x11  #211 pc 732
	add	%x10, %x11, %x10  #211 pc 736
	add	%x9, %x10, %x9  #211 pc 740
	add	%x8, %x9, %x8  #211 pc 744
	add	%x7, %x8, %x7  #211 pc 748
	add	%x7, %x7, %x15  #211 pc 752
	bge	%x0, %x6, 12  #212 pc 756
	j	ble_else.446 #pc 760
	nop #pc 764
	sub	%x6, %x0, %x6  #212 pc 768
	lw	%x30, 0(%x29)  #212 pc 772
	jalr	%x0, %x30, 0  #212 pc 776
	nop #pc 780
ble_else.446: #pc 784
	addi	%x6, %x7, 0  #212 pc 784
	ret #pc 788
	nop #pc 792
h.213:  #pc 796
	lw	%x7, 36(%x6)  #209 pc 796
	lw	%x8, 32(%x6)  #209 pc 800
	lw	%x9, 28(%x6)  #209 pc 804
	lw	%x10, 24(%x6)  #209 pc 808
	lw	%x11, 20(%x6)  #209 pc 812
	lw	%x12, 16(%x6)  #209 pc 816
	lw	%x13, 12(%x6)  #209 pc 820
	lw	%x14, 8(%x6)  #209 pc 824
	lw	%x15, 4(%x6)  #209 pc 828
	lw	%x6, 0(%x6)  #209 pc 832
	addi	%x29, %x3, 0  #210 pc 836
	addi	%x3, %x3, 44  #210 pc 840
	addi	%x16, %x0, 680  #210 g.239 pc 844
	sw	%x16, 0(%x29)  #210 pc 848
	sw	%x8, 40(%x29)  #210 pc 852
	sw	%x9, 36(%x29)  #210 pc 856
	sw	%x10, 32(%x29)  #210 pc 860
	sw	%x11, 28(%x29)  #210 pc 864
	sw	%x12, 24(%x29)  #210 pc 868
	sw	%x13, 20(%x29)  #210 pc 872
	sw	%x14, 16(%x29)  #210 pc 876
	sw	%x15, 12(%x29)  #210 pc 880
	sw	%x7, 8(%x29)  #210 pc 884
	sw	%x6, 4(%x29)  #210 pc 888
	addi	%x6, %x0, 1  #0 pc 892
	lw	%x30, 0(%x29)  #213 pc 896
	jalr	%x0, %x30, 0  #213 pc 900
	nop #pc 904
.global	min_caml_start
min_caml_start:
	addi	%x31, %x0, 1
	slli	%x4, %x31, 13
	slli	%x5, %x31, 16
	slli	%x2, %x31, 24
	add	%x3, %x2, %x4
	addi	%x6, %x0, 1  #0 pc 928
	addi	%x7, %x0, 2  #0 pc 932
	addi	%x8, %x0, 3  #0 pc 936
	addi	%x9, %x0, 4  #0 pc 940
	addi	%x10, %x0, 5  #0 pc 944
	addi	%x11, %x0, 6  #0 pc 948
	addi	%x12, %x0, 7  #0 pc 952
	addi	%x13, %x0, 8  #0 pc 956
	addi	%x14, %x0, 9  #0 pc 960
	addi	%x15, %x0, 10  #0 pc 964
	addi	%x16, %x3, 0  #214 pc 968
	addi	%x3, %x3, 40  #214 pc 972
	sw	%x15, 36(%x16)  #214 pc 976
	sw	%x14, 32(%x16)  #214 pc 980
	sw	%x13, 28(%x16)  #214 pc 984
	sw	%x12, 24(%x16)  #214 pc 988
	sw	%x11, 20(%x16)  #214 pc 992
	sw	%x10, 16(%x16)  #214 pc 996
	sw	%x9, 12(%x16)  #214 pc 1000
	sw	%x8, 8(%x16)  #214 pc 1004
	sw	%x7, 4(%x16)  #214 pc 1008
	sw	%x6, 0(%x16)  #214 pc 1012
	addi	%x6, %x16, 0  #214 pc 1016
	sw	%x1, 0(%x2)  #214 pc 1020
	addi	%x2, %x2, 4  #214 pc 1024
	jal	%x1, h.213  #214 pc 1028
	addi	%x2, %x2, -4  #214 pc 1032
	lw	%x1, 0(%x2) #214 pc 1036
	sw	%x1, 0(%x2)  #214 pc 1040
	addi	%x2, %x2, 4  #214 pc 1044
	jal	%x1, print_int.193  #214 pc 1048
	addi	%x2, %x2, -4  #214 pc 1052
	lw	%x1, 0(%x2) #214 pc 1056
	sw	%x1, 0(%x2)  #215 pc 1060
	addi	%x2, %x2, 4  #215 pc 1064
	jal	%x1, min_caml_print_newline  #215 pc 1068
	addi	%x2, %x2, -4  #215 pc 1072
	lw	%x1, 0(%x2) #215 pc 1076
