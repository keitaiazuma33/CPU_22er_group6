...
g.239.407:  #pc 536
	...
	j	ble_else.465 #pc 540
	nop #pc 544
	sub	%x6, %x0, %x6  #214 pc 548
	j	g.239.407  #214 pc 552
	nop #pc 556
ble_else.465: #pc 560
	addi	%x6, %x0, 55  #0 pc 560
	ret #pc 564
	nop #pc 568
...
min_caml_start:
	...
	slli	%x2, %x31, 24
	add	%x3, %x2, %x4
	addi	%x6, %x0, 1  #0 pc 592
	sw	%x1, 0(%x2)  #214 pc 596
	addi	%x2, %x2, 4  #214 pc 600
	jal	%x1, g.239.407  #214 pc 604
	addi	%x2, %x2, -4  #214 pc 608
	lw	%x1, 0(%x2) #214 pc 612
	sw	%x1, 0(%x2)  #214 pc 616
	addi	%x2, %x2, 4  #214 pc 620
	jal	%x1, print_int.193  #214 pc 624
	addi	%x2, %x2, -4  #214 pc 628
	lw	%x1, 0(%x2) #214 pc 632
	sw	%x1, 0(%x2)  #215 pc 636
	addi	%x2, %x2, 4  #215 pc 640
	jal	%x1, min_caml_print_newline  #215 pc 644
	addi	%x2, %x2, -4  #215 pc 648
	lw	%x1, 0(%x2) #215 pc 652
