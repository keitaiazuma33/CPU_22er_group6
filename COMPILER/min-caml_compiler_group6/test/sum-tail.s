...
be_else.379: #pc 124
	...
	j	div_abs.165  #81 pc 168
	nop #pc 172
ble_else.380: #pc 176
	addi	%x8, %x8, -1  #83 pc 176
	j	div_abs.165  #83 pc 180
	nop #pc 184
print_int.175:  #pc 188
	bge	%x6, %x0, 12  #100 pc 188
	j	bge_else.381 #pc 192
	nop #pc 196
	addi	%x31, %x0, 10  #pc 200
	bge	%x6, %x31, 12  #104 pc 204
	j	bge_else.382 #pc 208
	nop #pc 212
	bge	%x6, %x0, 12  #107 pc 216
	j	bge_else.383 #pc 220
	nop #pc 224
	addi	%x7, %x6, 0  #107 pc 228
	j	bge_cont.384 #pc 232
	nop #pc 236
bge_else.383: #pc 240
...
bge_else.381: #pc 512
	...
	j	print_int.175  #102 pc 528
	nop #pc 532
sum.195:  #pc 536
	bge	%x0, %x7, 12  #208 pc 536
	j	ble_else.393 #pc 540
	nop #pc 544
	ret #pc 548
	nop #pc 552
ble_else.393: #pc 556
	add	%x6, %x6, %x7  #209 pc 556
	addi	%x7, %x7, -1  #209 pc 560
	j	sum.195  #209 pc 564
	nop #pc 568
...
min_caml_start:
	...
	slli	%x2, %x31, 24
	add	%x3, %x2, %x4
	addi	%x6, %x0, 0  #0 pc 592
	addi	%x7, %x0, 10000  #0 pc 596
	sw	%x1, 0(%x2)  #210 pc 600
	addi	%x2, %x2, 4  #210 pc 604
	jal	%x1, sum.195  #210 pc 608
	addi	%x2, %x2, -4  #210 pc 612
	lw	%x1, 0(%x2) #210 pc 616
	sw	%x1, 0(%x2)  #210 pc 620
	addi	%x2, %x2, 4  #210 pc 624
	jal	%x1, print_int.175  #210 pc 628
	addi	%x2, %x2, -4  #210 pc 632
	lw	%x1, 0(%x2) #210 pc 636
