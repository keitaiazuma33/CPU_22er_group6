...
bge_cont.444: #pc 244
	...
	j	be_else.445 #pc 308
	nop #pc 312
	j	be_cont.446 #pc 316
	nop #pc 320
be_else.445: #pc 324
...
be_cont.446: #pc 328
	...
	j	bge_else.447 #pc 360
	nop #pc 364
	addi	%x7, %x6, 0  #109 pc 368
	j	bge_cont.448 #pc 372
	nop #pc 376
bge_else.447: #pc 380
...
bge_cont.448: #pc 384
	...
	j	be_else.449 #pc 444
	nop #pc 448
	j	be_cont.450 #pc 452
	nop #pc 456
be_else.449: #pc 460
...
bge_else.441: #pc 512
	...
	nop #pc 532
assign_farray.198:  #pc 536
	bge	%x7, %x0, 12  #188 pc 536
	j	bge_else.453 #pc 540
	nop #pc 544
	beq	%x7, %x0, 12  #191 pc 548
	j	be_else.454 #pc 552
	nop #pc 556
...
be_else.454: #pc 580
	...
	j	assign_farray.198  #195 pc 596
	nop #pc 600
bge_else.453: #pc 604
	ret #pc 604
	nop #pc 608
inprod.205:  #pc 612
	...
	j	bge_else.457 #pc 616
	nop #pc 620
	slli	%x9, %x8, 2  #209 pc 624
	add	%x31, %x9, %x6  #209 pc 628
	flw	%f1, 0(%x31)  #209 pc 632
	slli	%x9, %x8, 2  #209 pc 636
	add	%x31, %x9, %x7  #209 pc 640
	flw	%f2, 0(%x31)  #209 pc 644
	fmul	%f1, %f1, %f2  #209 pc 648
	fadd	%f0, %f0, %f1  #209 pc 652
	addi	%x8, %x8, -1  #209 pc 656
	j	inprod.205  #209 pc 660
	nop #pc 664
bge_else.457: #pc 668
	ret #pc 668
	nop #pc 672
...
min_caml_start:
	...
	slli	%x2, %x31, 24
	add	%x3, %x2, %x4
	fmv	%f0, l.419  #0 pc 696
	addi	%x6, %x3, 0  #210 pc 700
	addi	%x7, %x6, 0  #210 pc 704
	addi	%x6, %x6, 12  #210 pc 708
	addi	%x3, %x6, 0  #210 pc 712
	addi	%x6, %x0, 2  #0 pc 716
	sw	%x7, 0(%x2)  #210 pc 720
	addi	%x30, %x7, 0  #0 pc 724
	addi	%x7, %x6, 0  #0 pc 728
	addi	%x6, %x30, 0  #0 pc 732
	sw	%x1, 4(%x2)  #210 pc 736
	addi	%x2, %x2, 8  #210 pc 740
	jal	%x1, assign_farray.198  #210 pc 744
	addi	%x2, %x2, -8  #210 pc 748
	lw	%x1, 4(%x2) #210 pc 752
	fmv	%f0, l.421  #0 pc 756
	addi	%x6, %x3, 0  #211 pc 760
	addi	%x7, %x6, 0  #211 pc 764
	addi	%x6, %x6, 12  #211 pc 768
	addi	%x3, %x6, 0  #211 pc 772
	addi	%x6, %x0, 2  #0 pc 776
	sw	%x7, 4(%x2)  #211 pc 780
	addi	%x30, %x7, 0  #0 pc 784
	addi	%x7, %x6, 0  #0 pc 788
	addi	%x6, %x30, 0  #0 pc 792
	sw	%x1, 8(%x2)  #211 pc 796
	addi	%x2, %x2, 12  #211 pc 800
	jal	%x1, assign_farray.198  #211 pc 804
	addi	%x2, %x2, -12  #211 pc 808
	lw	%x1, 8(%x2) #211 pc 812
	fmv	%f0, l.423  #0 pc 816
	fmv	%f1, l.425  #0 pc 820
	addi	%x8, %x0, 2  #0 pc 824
	lw	%x6, 0(%x2)  #212 pc 828
	lw	%x7, 4(%x2)  #212 pc 832
	fsw	%f0, 8(%x2)  #212 pc 836
	fadd	%f0, %f1, %f30  #0 pc 840
	sw	%x1, 16(%x2)  #212 pc 844
	addi	%x2, %x2, 20  #212 pc 848
	jal	%x1, inprod.205  #212 pc 852
	addi	%x2, %x2, -20  #212 pc 856
	lw	%x1, 16(%x2) #212 pc 860
	flw	%f1, 8(%x2)  #212 pc 864
	fmul	%f0, %f1, %f0  #212 pc 868
	ftoi	%x6, %f0  #212 pc 872
	sw	%x1, 16(%x2)  #212 pc 876
	addi	%x2, %x2, 20  #212 pc 880
	jal	%x1, print_int.185  #212 pc 884
	addi	%x2, %x2, -20  #212 pc 888
	lw	%x1, 16(%x2) #212 pc 892
