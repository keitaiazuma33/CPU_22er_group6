...
bge_else.409: #pc 656
	...
	j	print_int.184  #102 pc 672
	nop #pc 676
assign_farray.197:  #pc 680
	bge	%x7, %x0, 12  #188 pc 680
	j	bge_else.413 #pc 684
	nop #pc 688
	beq	%x7, %x0, 12  #191 pc 692
	j	be_else.414 #pc 696
	nop #pc 700
...
be_else.414: #pc 724
	...
	j	assign_farray.197  #195 pc 740
	nop #pc 744
bge_else.413: #pc 748
	ret #pc 748
	nop #pc 752
create_float_array.201:  #pc 756
	...
	jal	%x1, assign_farray.197  #202 pc 796
...
inprod.204:  #pc 820
	...
	j	bge_else.417 #pc 824
	nop #pc 828
	slli	%x9, %x8, 2  #209 pc 832
	add	%x31, %x9, %x6  #209 pc 836
	flw	%f0, 0(%x31)  #209 pc 840
	slli	%x9, %x8, 2  #209 pc 844
	add	%x31, %x9, %x7  #209 pc 848
	flw	%f1, 0(%x31)  #209 pc 852
	fmul	%f0, %f0, %f1  #209 pc 856
	addi	%x8, %x8, -1  #209 pc 860
	fsw	%f0, 0(%x2)  #209 pc 864
	sw	%x1, 8(%x2)  #209 pc 868
	addi	%x2, %x2, 12  #209 pc 872
	jal	%x1, inprod.204  #209 pc 876
	addi	%x2, %x2, -12  #209 pc 880
	lw	%x1, 8(%x2) #209 pc 884
	flw	%f1, 0(%x2)  #209 pc 888
	fadd	%f0, %f1, %f0  #209 pc 892
	ret #pc 896
	nop #pc 900
bge_else.417: #pc 904
	fmv	%f0, l.372  #0 pc 904
	ret #pc 908
	nop #pc 912
...
min_caml_start:
	...
	slli	%x2, %x31, 24
	add	%x3, %x2, %x4
	addi	%x6, %x0, 3  #0 pc 936
	fmv	%f0, l.376  #0 pc 940
	sw	%x1, 0(%x2)  #210 pc 944
	addi	%x2, %x2, 4  #210 pc 948
	jal	%x1, create_float_array.201  #210 pc 952
	addi	%x2, %x2, -4  #210 pc 956
	lw	%x1, 0(%x2) #210 pc 960
	addi	%x7, %x0, 3  #0 pc 964
	fmv	%f0, l.378  #0 pc 968
	sw	%x6, 0(%x2)  #211 pc 972
	addi	%x6, %x7, 0  #0 pc 976
	sw	%x1, 4(%x2)  #211 pc 980
	addi	%x2, %x2, 8  #211 pc 984
	jal	%x1, create_float_array.201  #211 pc 988
	addi	%x2, %x2, -8  #211 pc 992
	lw	%x1, 4(%x2) #211 pc 996
	addi	%x7, %x6, 0  #211 pc 1000
	fmv	%f0, l.380  #0 pc 1004
	addi	%x8, %x0, 2  #0 pc 1008
	lw	%x6, 0(%x2)  #212 pc 1012
	fsw	%f0, 8(%x2)  #212 pc 1016
	sw	%x1, 16(%x2)  #212 pc 1020
	addi	%x2, %x2, 20  #212 pc 1024
	jal	%x1, inprod.204  #212 pc 1028
	addi	%x2, %x2, -20  #212 pc 1032
	lw	%x1, 16(%x2) #212 pc 1036
	flw	%f1, 8(%x2)  #212 pc 1040
	fmul	%f0, %f1, %f0  #212 pc 1044
	ftoi	%x6, %f0  #212 pc 1048
	sw	%x1, 16(%x2)  #212 pc 1052
	addi	%x2, %x2, 20  #212 pc 1056
	jal	%x1, print_int.184  #212 pc 1060
	addi	%x2, %x2, -20  #212 pc 1064
	lw	%x1, 16(%x2) #212 pc 1068
