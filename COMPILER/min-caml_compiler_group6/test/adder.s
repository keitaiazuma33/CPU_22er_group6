...
be_else.362: #pc 280
	...
	j	div_abs.162  #81 pc 324
	nop #pc 328
ble_else.363: #pc 332
	addi	%x8, %x8, -1  #83 pc 332
	j	div_abs.162  #83 pc 336
	nop #pc 340
div.167:  #pc 344
...
bge_else.369: #pc 656
	...
	j	print_int.172  #102 pc 672
	nop #pc 676
adder.198:  #pc 680
	lw	%x7, 4(%x29)  #208 pc 680
	add	%x6, %x7, %x6  #208 pc 684
	ret #pc 688
	nop #pc 692
make_adder.192:  #pc 696
	addi	%x7, %x3, 0  #208 pc 696
	addi	%x3, %x3, 8  #208 pc 700
	addi	%x8, %x0, 680  #208 adder.198 pc 704
	sw	%x8, 0(%x7)  #208 pc 708
	sw	%x6, 4(%x7)  #208 pc 712
	addi	%x6, %x7, 0  #209 pc 716
	ret #pc 720
	nop #pc 724
...
min_caml_start:
	...
	slli	%x2, %x31, 24
	add	%x3, %x2, %x4
	addi	%x6, %x0, 3  #0 pc 748
	sw	%x1, 0(%x2)  #210 pc 752
	addi	%x2, %x2, 4  #210 pc 756
	jal	%x1, make_adder.192  #210 pc 760
	addi	%x2, %x2, -4  #210 pc 764
	lw	%x1, 0(%x2) #210 pc 768
	addi	%x29, %x6, 0  #210 pc 772
	addi	%x6, %x0, 7  #0 pc 776
	sw	%x1, 0(%x2)  #210 pc 780
	lw	%x30, 0(%x29)  #210 pc 784
	addi	%x2, %x2, 4  #210 pc 788
	jalr	%x1, %x30, 0  #210 pc 792
	addi	%x2, %x2, -4  #210 pc 796
	lw	%x1, 0(%x2)  #210 pc 800
	sw	%x1, 0(%x2)  #210 pc 804
	addi	%x2, %x2, 4  #210 pc 808
	jal	%x1, print_int.172  #210 pc 812
