...
bge_else.373: #pc 656
	...
	nop #pc 676
f.195:  #pc 680
	lw	%x7, 4(%x29)  #0 pc 680
	beq	%x6, %x0, 12  #210 pc 684
	j	be_else.377 #pc 688
	nop #pc 692
	addi	%x6, %x0, 0  #0 pc 696
	ret #pc 700
	nop #pc 704
be_else.377: #pc 708
	addi	%x6, %x6, -1  #211 pc 708
	sw	%x7, 0(%x2)  #211 pc 712
	sw	%x1, 4(%x2)  #211 pc 716
	lw	%x30, 0(%x29)  #211 pc 720
	addi	%x2, %x2, 8  #211 pc 724
	jalr	%x1, %x30, 0  #211 pc 728
	addi	%x2, %x2, -8  #211 pc 732
	lw	%x1, 4(%x2)  #211 pc 736
	lw	%x7, 0(%x2)  #211 pc 740
	add	%x6, %x7, %x6  #211 pc 744
	ret #pc 748
	nop #pc 752
...
min_caml_start:
	...
	slli	%x2, %x31, 24
	add	%x3, %x2, %x4
	addi	%x6, %x0, 10  #0 pc 776
	addi	%x29, %x3, 0  #209 pc 780
	addi	%x3, %x3, 8  #209 pc 784
	addi	%x7, %x0, 680  #209 f.195 pc 788
	sw	%x7, 0(%x29)  #209 pc 792
	sw	%x6, 4(%x29)  #209 pc 796
	addi	%x6, %x0, 123  #0 pc 800
	sw	%x1, 0(%x2)  #212 pc 804
	lw	%x30, 0(%x29)  #212 pc 808
	addi	%x2, %x2, 4  #212 pc 812
	jalr	%x1, %x30, 0  #212 pc 816
	addi	%x2, %x2, -4  #212 pc 820
	lw	%x1, 0(%x2)  #212 pc 824
	sw	%x1, 0(%x2)  #212 pc 828
	addi	%x2, %x2, 4  #212 pc 832
	jal	%x1, print_int.174  #212 pc 836
	addi	%x2, %x2, -4  #212 pc 840
	lw	%x1, 0(%x2) #212 pc 844
