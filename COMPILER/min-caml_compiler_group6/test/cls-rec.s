...
be_else.381: #pc 124
	...
	j	div_abs.164  #81 pc 168
	nop #pc 172
ble_else.382: #pc 176
	addi	%x8, %x8, -1  #83 pc 176
	j	div_abs.164  #83 pc 180
	nop #pc 184
print_int.174:  #pc 188
	bge	%x6, %x0, 12  #100 pc 188
	j	bge_else.383 #pc 192
	nop #pc 196
	addi	%x31, %x0, 10  #pc 200
	bge	%x6, %x31, 12  #104 pc 204
	j	bge_else.384 #pc 208
	nop #pc 212
	bge	%x6, %x0, 12  #107 pc 216
	j	bge_else.385 #pc 220
	nop #pc 224
	addi	%x7, %x6, 0  #107 pc 228
	j	bge_cont.386 #pc 232
	nop #pc 236
bge_else.385: #pc 240
...
bge_else.383: #pc 512
	...
	j	print_int.174  #102 pc 528
	nop #pc 532
f.195:  #pc 536
	lw	%x7, 4(%x29)  #0 pc 536
	beq	%x6, %x0, 12  #210 pc 540
	j	be_else.395 #pc 544
	nop #pc 548
	addi	%x6, %x0, 0  #0 pc 552
	ret #pc 556
	nop #pc 560
be_else.395: #pc 564
	addi	%x6, %x6, -1  #211 pc 564
	sw	%x7, 0(%x2)  #211 pc 568
	sw	%x1, 4(%x2)  #211 pc 572
	lw	%x30, 0(%x29)  #211 pc 576
	addi	%x2, %x2, 8  #211 pc 580
	jalr	%x1, %x30, 0  #211 pc 584
	addi	%x2, %x2, -8  #211 pc 588
	lw	%x1, 4(%x2)  #211 pc 592
	lw	%x7, 0(%x2)  #211 pc 596
	add	%x6, %x7, %x6  #211 pc 600
	ret #pc 604
	nop #pc 608
...
min_caml_start:
	...
	slli	%x2, %x31, 24
	add	%x3, %x2, %x4
	addi	%x6, %x0, 10  #0 pc 632
	addi	%x29, %x3, 0  #209 pc 636
	addi	%x3, %x3, 8  #209 pc 640
	addi	%x7, %x0, 536  #209 f.195 pc 644
	sw	%x7, 0(%x29)  #209 pc 648
	sw	%x6, 4(%x29)  #209 pc 652
	addi	%x6, %x0, 123  #0 pc 656
	sw	%x1, 0(%x2)  #212 pc 660
	lw	%x30, 0(%x29)  #212 pc 664
	addi	%x2, %x2, 4  #212 pc 668
	jalr	%x1, %x30, 0  #212 pc 672
	addi	%x2, %x2, -4  #212 pc 676
	lw	%x1, 0(%x2)  #212 pc 680
	sw	%x1, 0(%x2)  #212 pc 684
	addi	%x2, %x2, 4  #212 pc 688
	jal	%x1, print_int.174  #212 pc 692
	addi	%x2, %x2, -4  #212 pc 696
	lw	%x1, 0(%x2) #212 pc 700
