...
be_else.422: #pc 124
	...
	j	div_abs.166  #81 pc 168
	nop #pc 172
ble_else.423: #pc 176
	addi	%x8, %x8, -1  #83 pc 176
	j	div_abs.166  #83 pc 180
	nop #pc 184
print_int.176:  #pc 188
...
be_cont.429: #pc 328
	...
	jal	%x1, print_int.176  #108 pc 340
...
bge_else.424: #pc 512
	...
	j	print_int.176  #102 pc 528
	nop #pc 532
assign_array.182:  #pc 536
	bge	%x8, %x0, 12  #169 pc 536
	j	bge_else.436 #pc 540
	nop #pc 544
	beq	%x8, %x0, 12  #172 pc 548
	j	be_else.437 #pc 552
	nop #pc 556
...
be_else.437: #pc 580
	...
	j	assign_array.182  #176 pc 596
	nop #pc 600
bge_else.436: #pc 604
	ret #pc 604
	nop #pc 608
f.196:  #pc 612
	bge	%x6, %x0, 12  #210 pc 612
	j	bge_else.440 #pc 616
	nop #pc 620
	sw	%x6, 0(%x2)  #211 pc 624
	sw	%x29, 4(%x2)  #211 pc 628
	sw	%x1, 8(%x2)  #211 pc 632
	addi	%x2, %x2, 12  #211 pc 636
	jal	%x1, print_int.176  #211 pc 640
	addi	%x2, %x2, -12  #211 pc 644
	lw	%x1, 8(%x2) #211 pc 648
	addi	%x6, %x3, 0  #212 pc 652
	addi	%x7, %x6, 0  #212 pc 656
	addi	%x6, %x6, 4  #212 pc 660
	addi	%x3, %x6, 0  #212 pc 664
	addi	%x8, %x0, 0  #0 pc 668
	lw	%x6, 4(%x2)  #212 pc 672
	sw	%x7, 8(%x2)  #212 pc 676
	addi	%x30, %x7, 0  #0 pc 680
	addi	%x7, %x6, 0  #0 pc 684
	addi	%x6, %x30, 0  #0 pc 688
	sw	%x1, 12(%x2)  #212 pc 692
	addi	%x2, %x2, 16  #212 pc 696
	jal	%x1, assign_array.182  #212 pc 700
	addi	%x2, %x2, -16  #212 pc 704
	lw	%x1, 12(%x2) #212 pc 708
	lw	%x6, 8(%x2)  #213 pc 712
	lw	%x29, 0(%x6)  #213 pc 716
	lw	%x6, 0(%x2)  #213 pc 720
	addi	%x6, %x6, -1  #213 pc 724
	lw	%x30, 0(%x29)  #213 pc 728
	jalr	%x0, %x30, 0  #213 pc 732
	nop #pc 736
bge_else.440: #pc 740
	ret #pc 740
	nop #pc 744
...
min_caml_start:
	...
	slli	%x2, %x31, 24
	add	%x3, %x2, %x4
	addi	%x6, %x3, 0  #209 pc 768
	addi	%x3, %x3, 4  #209 pc 772
	addi	%x7, %x0, 612  #209 f.196 pc 776
	sw	%x7, 0(%x6)  #209 pc 780
	addi	%x7, %x0, 9  #0 pc 784
	sw	%x6, 0(%x2)  #214 pc 788
	addi	%x6, %x7, 0  #0 pc 792
	sw	%x1, 4(%x2)  #214 pc 796
	addi	%x2, %x2, 8  #214 pc 800
	jal	%x1, print_int.176  #214 pc 804
	addi	%x2, %x2, -8  #214 pc 808
	lw	%x1, 4(%x2) #214 pc 812
	addi	%x6, %x3, 0  #214 pc 816
	addi	%x7, %x6, 0  #214 pc 820
	addi	%x6, %x6, 4  #214 pc 824
	addi	%x3, %x6, 0  #214 pc 828
	addi	%x8, %x0, 0  #0 pc 832
	lw	%x6, 0(%x2)  #214 pc 836
	sw	%x7, 4(%x2)  #214 pc 840
	addi	%x30, %x7, 0  #0 pc 844
	addi	%x7, %x6, 0  #0 pc 848
	addi	%x6, %x30, 0  #0 pc 852
	sw	%x1, 8(%x2)  #214 pc 856
	addi	%x2, %x2, 12  #214 pc 860
	jal	%x1, assign_array.182  #214 pc 864
	addi	%x2, %x2, -12  #214 pc 868
	lw	%x1, 8(%x2) #214 pc 872
	lw	%x6, 4(%x2)  #214 pc 876
	lw	%x29, 0(%x6)  #214 pc 880
	addi	%x6, %x0, 8  #0 pc 884
	sw	%x1, 8(%x2)  #214 pc 888
	lw	%x30, 0(%x29)  #214 pc 892
	addi	%x2, %x2, 12  #214 pc 896
	jalr	%x1, %x30, 0  #214 pc 900
	addi	%x2, %x2, -12  #214 pc 904
	lw	%x1, 8(%x2)  #214 pc 908
