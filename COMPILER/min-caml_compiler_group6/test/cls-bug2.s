...
bge_else.383: #pc 656
	...
	j	print_int.176  #102 pc 672
	nop #pc 676
assign_array.182:  #pc 680
	bge	%x8, %x0, 12  #169 pc 680
	j	bge_else.387 #pc 684
	nop #pc 688
	beq	%x8, %x0, 12  #172 pc 692
	j	be_else.388 #pc 696
	nop #pc 700
...
be_else.388: #pc 724
	...
	j	assign_array.182  #176 pc 740
	nop #pc 744
bge_else.387: #pc 748
	ret #pc 748
	nop #pc 752
create_array.186:  #pc 756
	...
	jal	%x1, assign_array.182  #183 pc 796
	...
	nop #pc 816
f.196:  #pc 820
	bge	%x6, %x0, 12  #210 pc 820
	j	bge_else.391 #pc 824
	nop #pc 828
	sw	%x6, 0(%x2)  #211 pc 832
	sw	%x29, 4(%x2)  #211 pc 836
	sw	%x1, 8(%x2)  #211 pc 840
	addi	%x2, %x2, 12  #211 pc 844
	jal	%x1, print_int.176  #211 pc 848
	addi	%x2, %x2, -12  #211 pc 852
	lw	%x1, 8(%x2) #211 pc 856
	addi	%x6, %x0, 1  #0 pc 860
	lw	%x7, 4(%x2)  #212 pc 864
	sw	%x1, 8(%x2)  #212 pc 868
	addi	%x2, %x2, 12  #212 pc 872
	jal	%x1, create_array.186  #212 pc 876
	addi	%x2, %x2, -12  #212 pc 880
	lw	%x1, 8(%x2) #212 pc 884
	lw	%x29, 0(%x6)  #213 pc 888
	lw	%x6, 0(%x2)  #213 pc 892
	addi	%x6, %x6, -1  #213 pc 896
	lw	%x30, 0(%x29)  #213 pc 900
	jalr	%x0, %x30, 0  #213 pc 904
	nop #pc 908
bge_else.391: #pc 912
	ret #pc 912
	nop #pc 916
...
min_caml_start:
	...
	slli	%x2, %x31, 24
	add	%x3, %x2, %x4
	addi	%x29, %x3, 0  #209 pc 940
	addi	%x3, %x3, 4  #209 pc 944
	addi	%x6, %x0, 820  #209 f.196 pc 948
	sw	%x6, 0(%x29)  #209 pc 952
	addi	%x6, %x0, 9  #0 pc 956
	sw	%x1, 0(%x2)  #214 pc 960
	lw	%x30, 0(%x29)  #214 pc 964
	addi	%x2, %x2, 4  #214 pc 968
	jalr	%x1, %x30, 0  #214 pc 972
	addi	%x2, %x2, -4  #214 pc 976
	lw	%x1, 0(%x2)  #214 pc 980
