...
bge_else.469: #pc 680
	...
	j	print_int.211  #102 pc 688
	nop #pc 692
assign_array.225:  #pc 692
	beq	%x7, %x0, 12  #160 pc 696
	j	be_else.473 #pc 700
	nop #pc 704
...
be_else.473: #pc 724
	...
	j	assign_array.225  #164 pc 744
	nop #pc 748
create_array.229:  #pc 748
	...
	jal	%x1, assign_array.225  #171 pc 792
	...
	nop #pc 812
f.239:  #pc 812
	bge	%x5, %x0, 12  #194 pc 816
	j	bge_else.475 #pc 820
	nop #pc 824
	sw	%x5, 0(%x2)  #195 pc 828
	sw	%x22, 4(%x2)  #195 pc 832
	sw	%x1, 12(%x2)  #195 pc 836
	addi	%x2, %x2, 16  #195 pc 840
	jal	%x1, print_int.211  #195 pc 844
	addi	%x2, %x2, -16  #195 pc 848
	lw	%x1, 12(%x2) #195 pc 852
	addi	%x5, %x0, 1  #0 pc 856
	lw	%x6, 4(%x2)  #196 pc 860
	sw	%x1, 12(%x2)  #196 pc 864
	addi	%x2, %x2, 16  #196 pc 868
	jal	%x1, create_array.229  #196 pc 872
	addi	%x2, %x2, -16  #196 pc 876
	lw	%x1, 12(%x2) #196 pc 880
	add	%x24, %x0, %x5  #197 pc 884
	lw	%x22, 0(%x24)  #197 pc 888
	lw	%x5, 0(%x2)  #197 pc 892
	addi	%x5, %x5, -1  #197 pc 896
	lw	%x23, 0(%x22)  #197 pc 900
	jalr	%x0, %x23, 0  #197 pc 904
	nop #pc 908
bge_else.475: #pc 908
	ret #pc 912
	nop #pc 916
...
min_caml_start:
	addi	%x2, %x2, -112
	addi	%x22, %x3, 0  #193 pc 920
	addi	%x3, %x3, 8  #193 pc 924
	addi	%x24, %x0, 812  #193 f.239 pc 928
	addi	%x5, %x24, 0  #193 pc 932
	add	%x24, %x0, %x22  #193 pc 936
	sw	%x5, 0(%x24)  #193 pc 940
	addi	%x5, %x0, 9  #0 pc 944
	sw	%x1, 12(%x2)  #198 pc 948
	lw	%x23, 0(%x22)  #198 pc 952
	addi	%x2, %x2, 16  #198 pc 956
	jalr	%x1, %x23, 0  #198 pc 960
	addi	%x2, %x2, -16  #198 pc 964
	lw	%x1, 12(%x2)  #198 pc 968
	addi	%x2, %x2, 112
