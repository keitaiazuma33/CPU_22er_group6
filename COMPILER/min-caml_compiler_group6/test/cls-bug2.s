...
bge_else.469: #pc 640
	...
	j	print_int.211  #102 pc 652
	nop #pc 656
assign_array.225:  #pc 660
	beq	%x7, %x0, 12  #160 pc 660
	j	be_else.473 #pc 664
	nop #pc 668
...
be_else.473: #pc 692
	...
	j	assign_array.225  #164 pc 708
	nop #pc 712
create_array.229:  #pc 716
	...
	jal	%x1, assign_array.225  #171 pc 756
	...
	nop #pc 776
f.239:  #pc 780
	bge	%x5, %x0, 12  #194 pc 780
	j	bge_else.475 #pc 784
	nop #pc 788
	sw	%x5, 0(%x2)  #195 pc 792
	sw	%x29, 4(%x2)  #195 pc 796
	sw	%x1, 8(%x2)  #195 pc 800
	addi	%x2, %x2, 12  #195 pc 804
	jal	%x1, print_int.211  #195 pc 808
	addi	%x2, %x2, -12  #195 pc 812
	lw	%x1, 8(%x2) #195 pc 816
	addi	%x5, %x0, 1  #0 pc 820
	lw	%x6, 4(%x2)  #196 pc 824
	sw	%x1, 8(%x2)  #196 pc 828
	addi	%x2, %x2, 12  #196 pc 832
	jal	%x1, create_array.229  #196 pc 836
	addi	%x2, %x2, -12  #196 pc 840
	lw	%x1, 8(%x2) #196 pc 844
	lw	%x29, 0(%x5)  #197 pc 848
	lw	%x5, 0(%x2)  #197 pc 852
	addi	%x5, %x5, -1  #197 pc 856
	lw	%x30, 0(%x29)  #197 pc 860
	jalr	%x0, %x30, 0  #197 pc 864
	nop #pc 868
bge_else.475: #pc 872
	ret #pc 872
	nop #pc 876
...
min_caml_start:
	addi	%x2, %x0, 0
	addi	%x3, %x0, 1024
	addi	%x29, %x3, 0  #193 pc 888
	addi	%x3, %x3, 4  #193 pc 892
	addi	%x31, %x0, 780  #193 f.239 pc 896
	addi	%x5, %x31, 0  #193 pc 900
	sw	%x5, 0(%x29)  #193 pc 904
	addi	%x5, %x0, 9  #0 pc 908
	sw	%x1, 0(%x2)  #198 pc 912
	lw	%x30, 0(%x29)  #198 pc 916
	addi	%x2, %x2, 4  #198 pc 920
	jalr	%x1, %x30, 0  #198 pc 924
	addi	%x2, %x2, -4  #198 pc 928
	lw	%x1, 0(%x2)  #198 pc 932
	addi	%x2, %x2, 112
