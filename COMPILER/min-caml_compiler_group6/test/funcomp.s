...
min_caml_start:
	...
	sw	%x6, 8(%x2)  #7 pc 240
	mv	%x6, %x7 #pc 244
	sw	%x1, 12(%x2)  #7 pc 248
	addi	%x2, %x2, 16  #7 pc 252
	jal	%x1, compose.7  #7 pc 256
	addi	%x2, %x2, -16  #7 pc 260
	lw	%x1, 12(%x2) #7 pc 264
	mv	%x6, %x5  #7 pc 268
	lw	%x5, 8(%x2)  #7 pc 272
	sw	%x1, 12(%x2)  #7 pc 276
	addi	%x2, %x2, 16  #7 pc 280
	jal	%x1, compose.7  #7 pc 284
	addi	%x2, %x2, -16  #7 pc 288
	lw	%x1, 12(%x2) #7 pc 292
	mv	%x22, %x5  #7 pc 296
	addi	%x5, %x0, 123  #0 pc 300
	sw	%x1, 12(%x2)  #8 pc 304
	lw	%x23, 0(%x22)  #8 pc 308
	addi	%x2, %x2, 16  #8 pc 312
	jalr	%x1, %x23, 0  #8 pc 316
	addi	%x2, %x2, -16  #8 pc 320
	lw	%x1, 12(%x2)  #8 pc 324
	sw	%x1, 12(%x2)  #8 pc 328
	addi	%x2, %x2, 16  #8 pc 332
	jal	%x1, min_caml_print_int  #8 pc 336
