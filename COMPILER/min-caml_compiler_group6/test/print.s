...
min_caml_start:
	...
	slli	%x2, %x31, 24
	add	%x3, %x2, %x4
	addi	%x6, %x0, 123  #0 pc 692
	sw	%x1, 0(%x2)  #207 pc 696
	addi	%x2, %x2, 4  #207 pc 700
	jal	%x1, print_int.176  #207 pc 704
	addi	%x2, %x2, -4  #207 pc 708
	lw	%x1, 0(%x2) #207 pc 712
	addi	%x6, %x0, -456  #0 pc 716
	sw	%x1, 0(%x2)  #208 pc 720
	addi	%x2, %x2, 4  #208 pc 724
	jal	%x1, print_int.176  #208 pc 728
	addi	%x2, %x2, -4  #208 pc 732
	lw	%x1, 0(%x2) #208 pc 736
	addi	%x6, %x0, 789  #0 pc 740
	sw	%x1, 0(%x2)  #209 pc 744
	addi	%x2, %x2, 4  #209 pc 748
	jal	%x1, print_int.176  #209 pc 752
	addi	%x2, %x2, -4  #209 pc 756
	lw	%x1, 0(%x2) #209 pc 760
