...
be_else.369: #pc 124
	...
	j	div_abs.159  #81 pc 168
	nop #pc 172
ble_else.370: #pc 176
	addi	%x8, %x8, -1  #83 pc 176
	j	div_abs.159  #83 pc 180
	nop #pc 184
print_int.169:  #pc 188
	bge	%x6, %x0, 12  #100 pc 188
	j	bge_else.371 #pc 192
	nop #pc 196
	addi	%x31, %x0, 10  #pc 200
	bge	%x6, %x31, 12  #104 pc 204
	j	bge_else.372 #pc 208
	nop #pc 212
	bge	%x6, %x0, 12  #107 pc 216
	j	bge_else.373 #pc 220
	nop #pc 224
	addi	%x7, %x6, 0  #107 pc 228
	j	bge_cont.374 #pc 232
	nop #pc 236
bge_else.373: #pc 240
...
min_caml_start:
	...
	slli	%x2, %x31, 24
	add	%x3, %x2, %x4
	addi	%x6, %x0, 6  #0 pc 556
	sw	%x1, 0(%x2)  #212 pc 560
	addi	%x2, %x2, 4  #212 pc 564
	jal	%x1, print_int.169  #212 pc 568
